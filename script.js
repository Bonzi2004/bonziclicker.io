document.addEventListener('DOMContentLoaded', () => {
    ...
    let lastClickTime = 0;
    let clickIntervals = [];
    const MAX_CLICKS_PER_SECOND = 15; // Максимальное количество кликов в секунду
    const BAN_DURATION = 3 * 24 * 60 * 60 * 1000; // 3 дня в миллисекундах
    ...
    function detectAutoclicker() {
        ...
        if (clickIntervals.length >= 5) {
            ...
            // Если скорость кликов слишком высокая и интервалы слишком регулярные
    ...
    function isTooRegular(intervals) {
        // Проверяем, насколько интервалы между кликами одинаковые
        const avg = intervals.reduce((a, b) => a + b, 0) / intervals.length;
        const variance = intervals.reduce((acc, interval) => {
            return acc + Math.pow(interval - avg, 2);
        }, 0) / intervals.length;

        return variance < 10; // Если отклонение очень маленькое - это автокликер
    }
    ...
    // Update click challenge progress
